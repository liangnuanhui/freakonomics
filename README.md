...
从播客 RSS 拉取列表并**默认同步下载音频 + 文稿**（断点续跑、`progress.json` 与官网路径一致）。  
RSS 文稿来自 feed 里的 description / show notes（写成同名 `.md`），**不是**官网全文 transcript。只要音频时用 `--no-transcript`。

```bash
# No Stupid Questions（快捷写法：音频 + 简介 Markdown 同步下载）
...
RSS 文件名：有集数时为 `{集数}-{标题}.mp3` / `.md`，否则 `{标题}.mp3` / `.md`。
...
| `--transcript` / `--no-transcript` | 官网：全文文稿；RSS：feed 简介写成 `.md`（与音频默认同开） | 开 |
